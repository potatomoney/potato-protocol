...
// all votes work on underlying _tatoBalances[address], not balanceOf(address)
...
//
// Changes made by TATO after audit:
//    Formatting, naming, & uint256 instatod of uint
//    Since TATO supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match TATO as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "TATO Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(tato.initSupply(), 4), 100); } // 4% of TATO

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(tato.initSupply(), 100); } // 1% of TATO
    ...
    constructor(address timelock_, address tato_) public {
        timelock = TimelockInterface(timelock_);
        tato = TATOInterface(tato_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(tato.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || tato.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = tato.getPriorVotes(voter, proposal.startBlock);
